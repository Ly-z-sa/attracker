...
async function initializeUser() {
    try {
        ...
        const userData = userDoc.exists ? userDoc.data() : { name: 'Student User', email: 'student@example.com' };
...
async function updateStats() {
    try {
        ...
        const attendanceSnapshot = await attendanceCollection.where('date', '==', today).get();
...
async function loadReports() {
    try {
        ...
        const weeklySnapshot = await attendanceCollection
            .where('date', '>=', weekStart.toISOString().split('T')[0])
            .get();
        
        let weeklyStats = { present: 0, absent: 0, late: 0 };
        ...
        const monthlySnapshot = await attendanceCollection
            .where('date', '>=', monthStart.toISOString().split('T')[0])
            .get();
        
        let monthlyStats = { present: 0, absent: 0, late: 0 };
        ...
        for (let doc of subjectsSnapshot.docs) {
            const subject = doc.data();
...
async function loadSubjects() {
    try {
        const snapshot = await subjectsCollection.orderBy('createdAt', 'desc').get();
